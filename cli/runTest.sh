...
echo_line
# (Floor > 2 and Floor < 5)
./oabe_enc -s CP -p org1 -e "((Doctor or Nurse) and (Floor in (2-5)))" -i ${INPUT} -o ${INPUT}.cpabe
...
./oabe_keygen -s KP -p org2 -i "(From:Alice and (Date = March 1-14, 2015 or Date = February 22-28, 2015))" -o analystKPABE
...
echo_line
./oabe_enc -s KP -p org2 -e "From:Alice|To:Bob|Date = March 7, 2015" -i ${INPUT} -o ${INPUT}1.kpabe
assert "Encrypt under three attributes" 0 $?

echo_line
./oabe_enc -s KP -p org2 -e "From:Alice|To:Charlie|Date = March 15, 2015" -i ${INPUT} -o ${INPUT}2.kpabe
assert "Encrypt under three attributes" 0 $?
