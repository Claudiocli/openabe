...
int runPkeKeygen(string& id, string& suffix) {
  ...
  try {
    ...
    WriteToFile(pubKeyFile.c_str(), PK_BEGIN_HEADER +
                                        Base64Encode(publicKey.getInternalPtr(), publicKey.size()) +
                                        PK_END_HEADER);
    WriteToFile(privKeyFile.c_str(),
                SK_BEGIN_HEADER + Base64Encode(privateKey.getInternalPtr(), privateKey.size()) +
                    SK_END_HEADER);
    err_code = 0;
...
int runAbeKeygen(OpenABE_SCHEME scheme_type, string& prefix, string& suffix, string& keyInput,
                 string& keyFile, string& userGlobID, bool verbose) {
  int err_code = -1;
  OpenABE_ERROR result = OpenABE_NOERROR;
  ...
  try {
    ...
    WriteToFile(keyFile.c_str(), SK_BEGIN_HEADER +
