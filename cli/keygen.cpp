...
#define USAGE                                                                  \
  ...
  "\t-i : key id for 'PK', attribute list for 'CP'/'MA' and policy string "    \
  "for 'KP'\n"                                                                 \
...
int runPkeKeygen(string &id, string &suffix) {
  ...
  try {
    ...
    if ((result = schemeContext->keygen(keyId, pkId, skId)) !=
        OpenABE_NOERROR) {
    ...
    WriteToFile(pubKeyFile.c_str(),
                PK_BEGIN_HEADER +
                    Base64Encode(publicKey.getInternalPtr(), publicKey.size()) +
                    PK_END_HEADER);
    WriteToFile(
        privKeyFile.c_str(),
        SK_BEGIN_HEADER +
            Base64Encode(privateKey.getInternalPtr(), privateKey.size()) +
            SK_END_HEADER);
    err_code = 0;
...
int runAbeKeygen(OpenABE_SCHEME scheme_type, string &prefix, string &suffix,
                 string &keyInput, string &keyFile, string &userGlobID,
                 bool verbose) {
  int err_code = -1;
  OpenABE_ERROR result = OpenABE_NOERROR;
  ...
  try {
    ...
    if ((result = schemeContext->loadMasterSecretParams(mskID, mskBlob)) !=
        OpenABE_NOERROR) {
      cerr << "unable to load the master secret parameters" << endl;
      throw result;
    }
    // generate the user's key
    if ((result = schemeContext->keygen(funcInput.get(), skID, mpkID, mskID)) !=
        OpenABE_NOERROR) {
    ...
    WriteToFile(keyFile.c_str(),
                SK_BEGIN_HEADER +
                    Base64Encode(skBlob.getInternalPtr(), skBlob.size()) +
                    SK_END_HEADER);
    err_code = 0;
  } catch (OpenABE_ERROR &error) {
    cout << "caught exception: " << OpenABE_errorToString(error) << endl;
...
int main(int argc, char **argv) {
  if (argc <= 1) {
    cout << OpenABE_CLI_STRING << "keygen utility, v"
         << (OpenABE_LIBRARY_VERSION / 100.) << endl;
    fprintf(stderr, USAGE);
    exit(-1);
  }
  int opt, status = 0;
  string scheme_type = "", prefix = "", suffix = "", funcInputStr = "",
         keyOutfile = "";
  ...
  if (scheme == OpenABE_SCHEME_PK_OPDH) {
    ...
    status = runAbeKeygen(scheme, prefix, suffix, funcInputStr, keyOutfile,
                          userGlobID, verbose);
  }
