...
void OpenABESymKeyAuthEnc::chooseRandomIV() {
...
OpenABE_ERROR OpenABESymKeyAuthEnc::encrypt(const string& plaintext, OpenABEByteString* iv,
  ...
  try {
    ...
    // cout << "Plaintext:\n";
    // BIO_dump_fp(stdout, (const char *) &((*plaintext)[0]),
    // plaintext->size()); cout << "Enc Key:\n"; BIO_dump_fp(stdout, (const char
    // *) this->key.getInternalPtr(), this->key.size());
