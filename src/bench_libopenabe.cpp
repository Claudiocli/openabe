  ...
  if (argc < 7) {
    ...
    cout << "Usage " << argv[0]
         << ": [ scheme => 'CP', 'KP' or 'MA' ] [ iterations ] [ attributes ] "
            ...
            "optional: timestamp in secs ]"
         << endl;
    cout << "\tscheme: the type of ABE scheme to benchmark" << endl;
    cout << "\titerations: the number of iterations per test" << endl;
    cout << "\tattributes: the number of attributes in the policy/attribute "
            "list for encryption"
         << endl;
    cout << "\tfixed or range: run with a fixed number of attributes or as a "
            "range from 1 to num. attributes"
         << endl;
    ...
    cout << "\ttimestamp: an optional timestamp (generated via 'date +%s' via "
         ...
         << endl;
    cout << "Benchmark Description: records the time to encrypt/decrypt a "
            "256-bit symmetric key using the CPA-KEM or CCA-KEM constructions."
         << endl;
    return -1;
  }
