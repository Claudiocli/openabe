...
string getBalancedOpenABETree(int start, int end) {
  if (start == end) {
    return createAttribute(start);
  }
  int mid = ceil((start + (end - start) / 2.0));
  if (mid == 0) {
    return "(" + createAttribute(start) + " and " + createAttribute(end) + ")";
  } else {
    return "(" + getBalancedOpenABETree(start, mid - 1) + " and " +
           getBalancedOpenABETree(mid, end) + ")";
  }
}
...
TEST_F(PolicyParser, InvalidExpInts) {
  ...
  TEST_DESCRIPTION(
      "Testing that integers in expint can be represented by number of bits specified");
...
TEST_F(PolicyParser, ExpIntNotAllowedInInputPolicy) {
  ...
  ASSERT_TRUE(createPolicyTree("Alice or foo_expint04_xxxxxxxxxxxxxxxxxxxxxxxxxxxxx0xx") ==
              nullptr);
}
...
TEST_F(LinearSecretSharing, TestOtherComparisonOps) {
  TEST_DESCRIPTION(
      "Testing LSSS with multiple comparison ops in addition to other attributes in the policy");
...
class Input {
public:
  Input(OpenABE_SCHEME scheme, const string enc_input, const string key, bool expect_pass,
        bool verbose = false) {
    scheme_type = scheme;
...
TEST_P(CPASecurityForSchemeTest, testWorkingExamples) {
  ...
  unique_ptr<OpenABEContextSchemeCPA> schemeContext =
      OpenABE_createContextABESchemeCPA(input.scheme_type); // OpenABE_SCHEME_CP_WATERS
  ...
  ASSERT_TRUE(schemeContext->encrypt(NULL, MPK, encInput.get(), &plaintext, &ciphertext) ==
              OpenABE_NOERROR);
  ...
  ASSERT_TRUE(schemeContext->keygen((OpenABEFunctionInput*)keyInput.get(), "DecKey", MPK, MSK) ==
              OpenABE_NOERROR);
...
TEST_P(CCASecurityForKEMTest, testWorkingExamples) {
  Input input = GetParam();
  TEST_DESCRIPTION("Testing CCA-secure KEM " + printScheme(input.scheme_type) +
                   " scheme with Key: '" + input.key_input + "' and Enc: '" + input.func_input +
                   "'");
  ...
  cout << "* Testing CCA KEM security for " << printScheme(input.scheme_type) << " schemes..."
       << endl;
  ...
  ASSERT_TRUE(ccaKEMContext->encryptKEM(rng_.get(), MPK, encInput.get(), DEFAULT_SYM_KEY_BYTES,
                                        sym_key, &ciphertext1) == OpenABE_NOERROR);
  ...
  ASSERT_TRUE(ccaKEMContext->generateDecryptionKey((OpenABEFunctionInput*)keyInput.get(),
                                                   "GoodDecKey1", MPK, MSK) == OpenABE_NOERROR);
  ...
  if (input.expect_pass_) {
    ASSERT_TRUE(ccaKEMContext->decryptKEM(MPK, "GoodDecKey1", &ciphertext1, DEFAULT_SYM_KEY_BYTES,
                                          sym_key1) == OpenABE_NOERROR);
    ASSERT_TRUE(*sym_key == *sym_key1);
  } else {
    ASSERT_FALSE(ccaKEMContext->decryptKEM(MPK, "GoodDecKey1", &ciphertext1, DEFAULT_SYM_KEY_BYTES,
                                           sym_key1) == OpenABE_NOERROR);
...
TEST_P(CCASecurityForSchemeTest, testWorkingExamples) {
  Input input = GetParam();
  TEST_DESCRIPTION("Testing CCA-secure scheme " + printScheme(input.scheme_type) +
                   " scheme with Key: '" + input.key_input + "' and Enc: '" + input.func_input +
                   "'");
  ...
  unique_ptr<OpenABEContextSchemeCCA> ccaSchemeContext =
      OpenABE_createContextABESchemeCCA(input.scheme_type);
  ...
  ASSERT_TRUE(ccaSchemeContext->encrypt(MPK, encInput.get(), pt, &ciphertext_1, &ciphertext_2) ==
              OpenABE_NOERROR);
  ...
  ASSERT_TRUE(ccaSchemeContext->keygen((OpenABEFunctionInput*)keyInput.get(), "GoodDecKey1", MPK,
                                       MSK) == OpenABE_NOERROR);
  ...
  string pt1;
  if (input.expect_pass_) {
    ASSERT_TRUE(ccaSchemeContext->decrypt(MPK, "GoodDecKey1", pt1, &ciphertext_1, &ciphertext_2) ==
                OpenABE_NOERROR);
    plaintext1 += pt1;
    ASSERT_TRUE(plaintext == plaintext1);
  } else {
    ASSERT_FALSE(ccaSchemeContext->decrypt(MPK, "GoodDecKey1", pt1, &ciphertext_1, &ciphertext_2) ==
                 OpenABE_NOERROR);
  }
}
...
INSTANTIATE_TEST_CASE_P(
    ABETest1, CPASecurityForSchemeTest,
    ::testing::Values(
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Alice|Charlie",
              true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Bob|David",
              true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Bob|Eve",
              false),
        Input(OpenABE_SCHEME_KP_GPSW, "Alice|Charlie",
              "((Alice or Bob) and (Charlie or David)) and Alice", true),
...
INSTANTIATE_TEST_CASE_P(
    ABETest2, CPASecurityForSchemeTest,
    ::testing::Values(
        ...
        Input(OpenABE_SCHEME_KP_GPSW, "Alice|Charlie|uid:567abcdef",
              "((Alice or Bob) and (Charlie or David)) and Alice", true),
        Input(OpenABE_SCHEME_KP_GPSW, "Alice|Charlie|uid:567abcdef", "((Alice or Bob) and Alice)",
              true),
        Input(OpenABE_SCHEME_KP_GPSW, "Alice|Charlie|uid:567abcdef",
              "((Alice and Bob) and Charlie)", false)));

INSTANTIATE_TEST_CASE_P(
    ABETest3, CPASecurityForSchemeTest,
    ::testing::Values(Input(OpenABE_SCHEME_CP_WATERS, "Alice and Date = May 1-10, 2016",
                            "Alice|Date=May 5, 2016", true),
                      Input(OpenABE_SCHEME_CP_WATERS, "Date = May 1-10, 2016 and (Alice or Bob)",
                            "Bob|Date=May 8, 2016", true),
                      Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and Date = May 1-10, 2016)",
                            "Bob|Eve|Date=May 12, 2016", false),
                      Input(OpenABE_SCHEME_KP_GPSW, "Charlie|Date = June 12, 2014",
                            "(Date = June 10-20, 2014 and (Charlie or David))", true),
                      Input(OpenABE_SCHEME_KP_GPSW, "David|Date = June 25, 2014",
                            "((David or Bob) and Date = June 21-28, 2014)", true),
                      Input(OpenABE_SCHEME_KP_GPSW, "Alice|Charlie|Date = June 30, 2014",
                            "((Alice and Date = June 21-28, 2014) and Charlie)", false)));

INSTANTIATE_TEST_CASE_P(
    ABETest4, CCASecurityForKEMTest,
    ::testing::Values(
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Alice|Charlie",
              true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Bob|David",
...
INSTANTIATE_TEST_CASE_P(
    ABETest5, CCASecurityForKEMTest,
    ::testing::Values(
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "uid:567abc", true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "Alice|Bob", true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "Bob|Eve", false),
...
INSTANTIATE_TEST_CASE_P(
    ABETest6, CCASecurityForKEMTest,
    ::testing::Values(Input(OpenABE_SCHEME_CP_WATERS, "Alice and Date = May 1-10, 2016",
                            "Alice|Date=May 5, 2016", true),
                      Input(OpenABE_SCHEME_CP_WATERS, "Date = May 1-10, 2016 and (Alice or Bob)",
                            "Bob|Date=May 8, 2016", true),
...
INSTANTIATE_TEST_CASE_P(
    ABETest7, CCASecurityForSchemeTest,
    ::testing::Values(
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Alice|Charlie",
              true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice or Bob) and (Charlie or David))", "Bob|David",
...
INSTANTIATE_TEST_CASE_P(
    ABETest8, CCASecurityForSchemeTest,
    ::testing::Values(
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "uid:567abc", true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "Alice|Bob", true),
        Input(OpenABE_SCHEME_CP_WATERS, "((Alice and Bob) or uid:567abc)", "Bob|Eve", false),
...
INSTANTIATE_TEST_CASE_P(
    ABETest9, CCASecurityForSchemeTest,
    ::testing::Values(Input(OpenABE_SCHEME_CP_WATERS, "Alice and Date = May 1-10, 2016",
                            "Alice|Date=May 5, 2016", true),
                      Input(OpenABE_SCHEME_CP_WATERS, "Date = May 1-10, 2016 and (Alice or Bob)",
                            "Bob|Date=May 8, 2016", true),
...
INSTANTIATE_TEST_CASE_P(
    CheckSat1, CheckIfSatisfiedTests,
    ::testing::Values(
        ...
        SatInput("(David or Charlie)", "Alice|Bob", false), SatInput("Bar", "Alice|Bob", false),
        SatInput("Alice", "Alice|Bob", true), SatInput("Foor", "Bar", false),
        // test uids
