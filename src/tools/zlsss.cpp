 ...
 *
 * @param[in] input     - OpenABEFunctionInput object describing the access
 structure
...
void OpenABELSSS::shareSecret(const OpenABEFunctionInput* input, ZP& elt) {
  ...
  //      OpenABE_LOG_AND_THROW("Could not share secret",
 ...
 * @param[in] attrList  - OpenABEAttributeList object describing the attribute
 * list
 ...
 *
 * @param[in] left      - OpenABEFunctionInput object describing the access
 * structure
...
bool OpenABELSSS::performCoefficientRecovery(OpenABEPolicy* policy,
  ...
  // First, scan down through the tree to identify which leaves must be
  // satisfied in order to recover the secret. We will need to compute one
  // coefficient for each leaf. The result is stored as a 'mark' in each tree
  // node -- from the leaves up to the root.
  ...
  // computing the necessary coefficients at each stage. When we hit a leaf
  // node, the coefficient will be added to the m_ResultMap result list.
  ZP one;
...
/*!
 * Utility routine (iterative version). Given an access structure (policy) and
 * an element, perform secret sharing on the given element.
 *
 * @param[in] treeNode      - OpenABETreeNode for the subtree
 * @param[in] elt           - ZP to be shared
 * @return                  - bool indicating success/failure
 * @throw                   - an exception if there is a problem sharing the
 * element
 */
  ...
  do {
    ...
    if (visitedNode->getNodeType() == GATE_TYPE_LEAF) {
      ...
      for (uint32_t i = 0; i < totalSubnodes; i++) {
        // Evaluate the polynomial at point (i+1) and recurse on the resulting
        // value
...
/*!
 * Utility routine (iterative version). Given an access structure (policy) where
 * each node has been 'marked' if it's necessary to recover the secret, move
 * through and calculate the coefficients for each leaf node.
 *
 * @param[in] treeNode      - OpenABETreeNode for the subtree
...
bool OpenABELSSS::iterativeCoefficientRecover(OpenABETreeNode* treeNode, ZP& inCoeff) {
  ...
  do {
    ...
    // Base case:
    // If the node is a leaf node, simply add the input coefficient  to the
    // results and return.
    if (visitedNode->getNodeType() == GATE_TYPE_LEAF) {
...
ZP OpenABELSSS::calculateCoefficient(OpenABETreeNode* treeNode, uint32_t index, uint32_t threshold,
  ...
  // Product for all marked subnodes (excluding index) of ( (0 - (X(i))) /
  // (X(subnode_index) - (X(i))) ) Note that X(i) = i+1.
  for (uint32_t i = 0; i < threshold; i++) {
 ...
 *
 * @param[in] treeNode      - OpenABETreeNode object containing label of the
 * leaf node
...
/*!
 * Evaluate a polynomial at a specified (integer) x coordinate. Takes a list of
 * coefficients and the x-coord as a uint32_t.
 *
 * @param[in] coefficients      - OpenABEElementList object containing the
 * coordinates
...
ZP OpenABELSSS::evaluatePolynomial(OpenABEElementList& coefficients, uint32_t x) {
  ...
  //  {
  //    OpenABE_LOG_AND_THROW("Coefficients vector has no elements",
  //    OpenABE_ERROR_SECRET_SHARING_FAILED);
  //  }
...
/*!
 * Utility routine (iterative version). Given an attribute list, scan the entire
 * tree, 'marking' nodes that are required to satisfy the policy. Returns the
 * number of leaves required to satisfy this subtree.
 *
 * @param[in] treeNode         - root of the subtree
 * @return                     - number of leaves required to satisfy this
 * subtree
 */
...
bool iterativeScanTree(OpenABETreeNode* treeNode, OpenABEAttributeList* attributeList) {
  ...
  while (!nodes.empty()) {
    ...
    if (allSubnodesVisited && isInternalNode) {
      ...
      // bool internal_node_satisfied = determineIfNodeShouldBeMarked(threshold,
      // topNode); if (internal_node_satisfied) {
      //    cout << "Gate satisfied. Num matches: " <<
      //    topNode->getNumSatisfied() endl;
      // }
      topNode->m_Visited = true;
      nodes.pop();
    }
...
bool determineIfNodeShouldBeMarked(uint32_t threshold, OpenABETreeNode* node) {
  ...
  if (node->getNodeType() == GATE_TYPE_AND) {
    ...
    for (size_t k = 0; k < list.size(); k++) {
      ...
      //        ", satisfied: " << (node->getSubnode(i)->getMark() ? "true":
      //        "false") << endl;
...
ZP OpenABELSSS::LSSStestSecretRecovery(const OpenABELSSSRowMap& coefficients,
  ...
  for (OpenABELSSSRowMap::const_iterator shareIt = shares.begin(); shareIt != shares.end();
    ...
    if (coeffIt == coefficients.end()) {
      ...
      // occurs in one of two situations: 1) there is legitimately a missing
      // coefficient in the list (due to recoveryCoefficient error) 2) we are
      // dealing with an OR policy, in which case only 1 of the coefficients is
      // necessary to recover secret
      continue;
    }
