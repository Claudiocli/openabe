...
OpenABE_ERROR OpenABEContextPKSIG::keygen(const std::string &pkID,
  ...
  try {
    ...
    params[0] = OSSL_PARAM_construct_utf8_string(
        OSSL_PKEY_PARAM_GROUP_NAME, const_cast<char *>(this->curveName.c_str()),
        0);
...
OpenABE_ERROR OpenABEContextPKSIG::sign(OpenABEPKey *privKey,
  ...
  if (EVP_DigestSignInit(md, NULL, EVP_sha256(), NULL, privKey->getPkey()) !=
      1) {
    error_msg = "EVP_DigestSignInit";
    goto out;
  }

  if (EVP_DigestSignUpdate(md, message->getInternalPtr(), message->size()) !=
      1) {
...
OpenABE_ERROR OpenABEContextPKSIG::verify(OpenABEPKey *pubKey,
                                          OpenABEByteString *message,
                                          OpenABEByteString *signature) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  EVP_MD_CTX *md = NULL;
  ...
  if (EVP_DigestVerifyInit(md, NULL, EVP_sha256(), NULL, pubKey->getPkey()) !=
      1) {
    error_msg = "EVP_DigestVerifyInit";
    goto out;
  }

  if (EVP_DigestVerifyUpdate(md, message->getInternalPtr(), message->size()) !=
      1) {
  ...
  answer =
      (EVP_DigestVerifyFinal(md, (unsigned char *)signature->getInternalPtr(),
                             signature->size()) == 1);
  if (!answer) {
...
bool OpenABEContextPKSIG::validatePublicKey(
    const shared_ptr<OpenABEPKey> &key) {
  ASSERT_NOTNULL(key);
  return this->validatePkey(key->getPkey(), false);
}

bool OpenABEContextPKSIG::validatePrivateKey(
    const std::shared_ptr<OpenABEPKey> &key) {
  ASSERT_NOTNULL(key);
...
OpenABEContextSchemePKSIG::OpenABEContextSchemePKSIG(
    unique_ptr<OpenABEContextPKSIG> pksig)
    : ZObject() {
  m_PKSIG = move(pksig);
}
...
OpenABE_ERROR
OpenABEContextSchemePKSIG::loadPrivateKey(const std::string &keyID,
                                          OpenABEByteString &keyBlob) {
  OpenABE_ERROR result = OpenABE_NOERROR;
...
OpenABE_ERROR
OpenABEContextSchemePKSIG::loadPublicKey(const std::string &keyID,
                                         OpenABEByteString &keyBlob) {
  OpenABE_ERROR result = OpenABE_NOERROR;
...
OpenABE_ERROR OpenABEContextSchemePKSIG::keygen(const std::string &pkID,
                                                const std::string &skID) {
  return this->m_PKSIG->keygen(pkID, skID);
}

OpenABE_ERROR OpenABEContextSchemePKSIG::sign(const std::string &skID,
                                              OpenABEByteString *message,
                                              OpenABEByteString *signature) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  shared_ptr<OpenABEPKey> SK = nullptr;
  ...
  try {
    ...
    SK = static_pointer_cast<OpenABEPKey>(
        this->m_PKSIG->getKeystore()->getSecretKey(skID));
    ASSERT_NOTNULL(SK);
...
OpenABE_ERROR OpenABEContextSchemePKSIG::verify(const std::string &pkID,
                                                OpenABEByteString *message,
                                                OpenABEByteString *signature) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  shared_ptr<OpenABEPKey> PK = nullptr;
  ...
  try {
    ...
    PK = static_pointer_cast<OpenABEPKey>(
        this->m_PKSIG->getKeystore()->getPublicKey(pkID));
    ASSERT_NOTNULL(PK);
