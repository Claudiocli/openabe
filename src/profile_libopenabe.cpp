...
int profileABE_CPA_KEM(OpenABE_SCHEME scheme_type, string operation,
                       string input_str) {
  ...
  if (scheme_type == OpenABE_SCHEME_CP_WATERS ||
    ...
    EXIT("ERROR: not one of the supported scheme types. See help menu for "
         "options.");
  }
  ...
  if (operation == KEYGEN_OP) {
    ...
    if (scheme_type == OpenABE_SCHEME_CP_WATERS) {
      keyFuncInput =
          std::unique_ptr<OpenABEFunctionInput>(createAttributeList(input_str));
      if (!keyFuncInput) {
        EXIT("ERROR: invalid key input string for CP - expecting attribute "
             "list");
      }

      const vector<string> *raw_attrs =
          ((OpenABEAttributeList *)keyFuncInput.get())->getAttributeList();
      policy_components = raw_attrs->size();

    } else if (scheme_type == OpenABE_SCHEME_KP_GPSW) {
      keyFuncInput =
          std::unique_ptr<OpenABEFunctionInput>(createPolicyTree(input_str));
      if (!keyFuncInput) {
        EXIT("ERROR: invalid key input string for KP - expecting policy");
      }

      set<string> attr_set =
          ((OpenABEPolicy *)keyFuncInput.get())->getAttrCompleteSet();
      policy_components = attr_set.size();
    }

    // let's generate the key then export
    context->generateDecryptionKey((OpenABEFunctionInput *)keyFuncInput.get(),
                                   "deckey", mpk, msk);
    ...
    if (scheme_type == OpenABE_SCHEME_CP_WATERS) {
      ...
      set<string> attr_set =
          ((OpenABEPolicy *)encFuncInput.get())->getAttrCompleteSet();
      policy_components = attr_set.size();
    } else if (scheme_type == OpenABE_SCHEME_KP_GPSW) {
      ...
      encFuncInput =
          std::unique_ptr<OpenABEFunctionInput>(createAttributeList(input_str));
      if (!encFuncInput) {
        EXIT("ERROR: invalid enc input string for KP - expecting attribute "
             "list");
      }

      const vector<string> *raw_attrs =
          ((OpenABEAttributeList *)encFuncInput.get())->getAttributeList();
      policy_components = raw_attrs->size();
    }

    symkey.reset(new OpenABESymKey);
    ciphertext.reset(new OpenABECiphertext);
    context->encryptKEM(nullptr, mpk, encFuncInput.get(), DEFAULT_SYM_KEY_BYTES,
                        symkey, ciphertext.get());
    ...
    EXIT("ERROR: you specified an invalid or unsupported operation. See help "
         "menu for options.");
  }
...
int profileABE_CCA_KEM(OpenABE_SCHEME scheme_type, string operation,
                       string input_str) {
  cout << "Profiling... " << getSchemeString(scheme_type) << endl;
  string mpk, msk, auth1mpk, auth1msk, decKey = "decKey";
  std::unique_ptr<OpenABEFunctionInput> keyFuncInput = nullptr,
  ...
  if (operation == KEYGEN_OP) {
    ...
    if (scheme_type == OpenABE_SCHEME_CP_WATERS) {
      keyFuncInput =
          std::unique_ptr<OpenABEFunctionInput>(createAttributeList(input_str));
      if (!keyFuncInput) {
        EXIT("ERROR: invalid key input string for CP/MA - expecting attribute "
             "list");
      }
    ...
    // let's generate the key then export
    ccaContext->generateDecryptionKey(
        (OpenABEFunctionInput *)keyFuncInput.get(), "deckey", mpk, msk);
...
int main(int argc, const char *argv[]) {
  ...
  if (argc < 5) {
    cout << "OpenABE profiler utility, v" << (OpenABE_LIBRARY_VERSION / 100.)
         << endl;
    cout << "Usage " << argv[0]
         << ": [ scheme => 'CP' or 'KP'] [ operation ] [ input ] [ 'cpa' or "
            "'cca']"
         << endl;
    cout << "\tscheme: the type of ABE scheme to benchmark" << endl;
    cout << "\toperation: 'keygen' or 'encrypt'" << endl;
    cout << "\tinput: an attribute list or policy tree string depending on "
            "scheme type"
         << endl;
