...
int profileABE_CPA_KEM(OpenABE_SCHEME scheme_type, string operation, string input_str) {
  ...
  if (operation == KEYGEN_OP) {
    ...
    context->encryptKEM(nullptr, mpk, encFuncInput.get(), DEFAULT_SYM_KEY_BYTES, symkey,
                        ciphertext.get());
...
int profileABE_CCA_KEM(OpenABE_SCHEME scheme_type, string operation, string input_str) {
  ...
  if (operation == KEYGEN_OP) {
    ...
    // let's generate the key then export
    ccaContext->generateDecryptionKey((OpenABEFunctionInput*)keyFuncInput.get(), "deckey", mpk,
                                      msk);
    ...
    symkey.reset(new OpenABESymKey);
    ciphertext.reset(new OpenABECiphertext);
    ccaContext->encryptKEM(rng2.get(), mpk, encFuncInput.get(), DEFAULT_SYM_KEY_BYTES, symkey,
                           ciphertext.get());
...
int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    if (secType == CPA) {
      ...
      cout << "Invalid security type! Expected Argument: '" << CPA << "' or '" << CCA << "'"
           << endl;
      return -1;
    }
