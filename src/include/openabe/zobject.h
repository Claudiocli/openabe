...
public:
  ...
  virtual ZObject& operator=(const ZObject& rhs) {
    return *this;
  }
  virtual ZObject* clone() const {
    return NULL;
  } // throw an exception if not implemented
  virtual void serialize(OpenABEByteString& result) const {
    throw OpenABE_ERROR_NOT_IMPLEMENTED;
  }
  virtual bool isEqual(ZObject* z) const {
    return false;
  }
