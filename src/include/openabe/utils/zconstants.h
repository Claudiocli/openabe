...
typedef enum _zGroupType { GROUP_NONE, GROUP_G1, GROUP_G2, GROUP_GT, GROUP_ZP } zGroupType;
...
#define ASSERT_GROUP(G, A, B)                                                                      \
  if ((A) != (G) || (B) != (G))                                                                    \
    throw oabe::OpenABE_ERROR_WRONG_GROUP;
#define ASSERT_RNG(R)                                                                              \
  if ((R) < 1)                                                                                     \
    throw oabe::OpenABE_ERROR_RAND_INSUFFICIENT;
#define ASSERT_PAIRING(P)                                                                          \
  if ((P) == NULL)                                                                                 \
    throw oabe::OpenABE_ERROR_WRONG_GROUP;
#define ASSERT_NOTNULL(A)                                                                          \
  if ((A) == NULL)                                                                                 \
    throw oabe::OpenABE_ERROR_INVALID_INPUT;
#define ASSERT_MESSAGE(A, B, C)                                                                    \
  if ((A) == false) {                                                                              \
    string tmp_s = B;                                                                              \
    fprintf(stderr, "%s:%s:%d: %s - '%s'\n", __FILE__, __FUNCTION__, __LINE__, tmp_s.c_str(),      \
            OpenABE_errorToString(C));                                                             \
    throw C;                                                                                       \
  }
#define ASSERT(A, B)                                                                               \
  if ((A) == false) {                                                                              \
    fprintf(stderr, "%s:%s:%d: '%s'\n", __FILE__, __FUNCTION__, __LINE__,                          \
            OpenABE_errorToString(B));                                                             \
    throw B;                                                                                       \
  }
#define THROW_ERROR(B)                                                                             \
  fprintf(stderr, "%s:%s:%d: '%s'\n", __FILE__, __FUNCTION__, __LINE__, OpenABE_errorToString(B)); \
  throw B;
