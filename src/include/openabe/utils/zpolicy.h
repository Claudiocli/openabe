...
public:
  ...
  uint32_t getNumSubnodes() {
    return this->m_Subnodes.size();
  }
  const bool getMark() const {
    return this->m_Mark;
  }
  const int getNumSatisfied() const {
    return this->m_Satisfied;
  }
  ...
  const uint32_t getNodeType() const {
    return this->m_nodeType;
  }
  void setNodeType(zGateType type) {
  ...
  void setThresholdValue(uint32_t k) {
    if (this->m_Subnodes.size() > 0) {
      this->m_thresholdValue = k;
    }
  }
...
public:
  ...
  OpenABETreeNode* getRootNode() const {
    return this->m_rootNode.get();
  }
  OpenABEPolicy* clone() const {
    return new OpenABEPolicy(*this);
  }
  ...
  bool getRevocationStatus() {
    return this->m_enabledRevocation;
  }
  void enableRevocation() {
    this->m_enabledRevocation = true;
  }
...
std::unique_ptr<OpenABEPolicy> addToRootOfInput(zGateType type, const std::string attribute,
