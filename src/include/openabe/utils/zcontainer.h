...
public:
  ...
  void setGroup(std::shared_ptr<ZGroup> group) {
    this->group = group;
  }
  ...
  ZP* getZP(const std::string& name) {
    return dynamic_cast<ZP*>(this->getComponent(name));
  }
  G1* getG1(const std::string& name) {
    return dynamic_cast<G1*>(this->getComponent(name));
  }
  G2* getG2(const std::string& name) {
    return dynamic_cast<G2*>(this->getComponent(name));
  }
  GT* getGT(const std::string& name) {
    return dynamic_cast<GT*>(this->getComponent(name));
  }

  ZP_t* getZP_t(const std::string& name) {
    return dynamic_cast<ZP_t*>(this->getComponent(name));
  }
  G_t* getG_t(const std::string& name) {
    return dynamic_cast<G_t*>(this->getComponent(name));
  }

  OpenABEByteString* getByteString(const std::string& name) {
    return dynamic_cast<OpenABEByteString*>(this->getComponent(name));
  }
  OpenABEUInteger* getInteger(const std::string& name) {
    return dynamic_cast<OpenABEUInteger*>(this->getComponent(name));
  }
...
inline std::string OpenABEMakeElementLabel(std::string base, std::string unique) {
