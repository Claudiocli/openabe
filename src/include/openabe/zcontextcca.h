...
public:
  ...
  void setSchemeType(OpenABE_SCHEME scheme_type) {
    this->abeSchemeContext->setSchemeType(scheme_type);
  }
  OpenABE_SCHEME getSchemeType() {
    return this->abeSchemeContext->getSchemeType();
  }
...
public:
  ...
  OpenABE_ERROR encryptKEM(OpenABERNG* rng, const std::string& mpkID,
...
public:
  ...
  OpenABEKeystore* getKeystore() const {
    return this->m_KEM_->getKeystore();
  }
  OpenABE_SCHEME getSchemeType() const {
  ...
  OpenABE_ERROR encrypt(const std::string& mpkID, const OpenABEFunctionInput* encryptInput,
                        const std::string& plaintext, OpenABECiphertext* ciphertext1,
...
public:
  ...
  OpenABE_ERROR keygen(OpenABEFunctionInput* keyInput, const std::string& keyID,
                       const std::string& mpkID, const std::string& mskID,
                       const std::string& gpkID = "", const std::string& GID = "");
  std::unique_ptr<oabe::crypto::OpenABESymKeyHandle>
  encrypt(const std::string& mpkID, const OpenABEFunctionInput* encryptInput,
          OpenABECiphertext* ciphertext);
  std::unique_ptr<oabe::crypto::OpenABESymKeyHandle>
  decrypt(const std::string& mpkID, const std::string& keyID, OpenABECiphertext* ciphertext);
};
