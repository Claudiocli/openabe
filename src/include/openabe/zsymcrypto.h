...
public:
  ...
  bool decrypt(std::string& plaintext, OpenABEByteString* iv, OpenABEByteString* ciphertext,
               OpenABEByteString* tag);
};

class OpenABESymKeyHandle {
public:
  virtual void encrypt(std::string& ciphertext, const std::string& plaintext) = 0;
  virtual void decrypt(std::string& plaintext, const std::string& ciphertext) = 0;
...
public:
  ...
  OpenABESymKeyHandleImpl(const std::string& keyBytes, bool apply_b64_encode = false);
  OpenABESymKeyHandleImpl(OpenABEByteString& keyBytes, OpenABEByteString& authData,
