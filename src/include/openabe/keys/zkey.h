...
public:
  OpenABEKey();
  OpenABEKey(const OpenABECurveID curveID, uint8_t algorithmID, const std::string ID,
             OpenABEByteString* uid = NULL);
  ~OpenABEKey();
  ...
  uint8_t getAlgorithmID() {
    return this->algorithmID;
  }
  uint8_t getLibID() {
    return this->libraryVersion;
  }
  OpenABEByteString& getUID() {
    return this->uid;
  }
  std::string getID() {
    return this->ID;
  }
  OpenABEKeyType getKeyType() {
