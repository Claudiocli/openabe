...
public:
  OpenABEKDF(uint8_t hashPrefix = KDF_HASH_FUNCTION_PREFIX, uint32_t hashLen = SHA2_BITLEN,
             uint32_t maxInputLen = OpenABE_MAX_KDF_BITLENGTH);
  ~OpenABEKDF();

  OpenABEByteString DeriveKey(OpenABEByteString& Z, uint32_t keyBitLen,
...
OpenABEByteString OpenABEPBKDF(OpenABEByteString& password, uint32_t keydataLenBytes,
                               OpenABEByteString& salt,
                               int iterationCount = OpenABE_KDF_ITERATION_COUNT);
