...
TEST_P(KeystoreManagerTest, testBaseCases) {
  ...
  TEST_DESCRIPTION("Testing keystore manager works with basic test cases for " +
                   printScheme(input.scheme_type));
...
INSTANTIATE_TEST_CASE_P(
    ABETest5, KeystoreManagerTest,
    // arg 1 = scheme type, arg 2 = key input list, arg 3 = enc input string
    ::testing::Values(Config(OpenABE_SCHEME_CP_WATERS,
                             "Alice|Bob:Bob|David:Frank|Eve|Alice|Charlie",
                             "((Alice or Bob) and (Charlie or David))"),
                      Config(OpenABE_SCHEME_KP_GPSW,
                             "((Alice or Charlie) and Eve):(David or Charlie):(Alice and Bob)",
