...
OpenABEPolicy::OpenABEPolicy()
    : OpenABEFunctionInput(), m_rootNode(nullptr), m_hasDuplicates(false),
      m_enabledRevocation(false) {
...
void OpenABEPolicy::setDuplicateInfo(std::map<std::string, int>& attr_count,
  ...
  if (attr_list.size() > 0) {
    ...
    for (auto it = attr_list.begin(); it != attr_list.end(); ++it) {
      // copy over data for the duplicate attributes only (ignore everything
      // else)
      this->m_attrDuplicateCount[*it] = attr_count[*it];
      // cout << "Duplicate: " << *it << " => " <<
      // this->m_attrDuplicateCount[*it] << endl;
    }
  }
...
uint32_t OpenABETreeNode::getThresholdValue() {
  uint32_t result = 0;
 ...
 *
 */
string OpenABETreeNode::toString() {
  string op = "";
  string tree = "";
  stringstream tmp;
