...
/// \file   zkeymgr.cpp
///
/// \brief  Class implementation for the OpenABE keystore manager (keystore
/// patent).
///
...
map<string, string> OpenABEKeystoreManager::getActiveUsers() {
  return activeUsers_;
}
...
vector<std::string> OpenABEKeystoreManager::deleteKeyCommand(OpenABEKeyQuery* query) {
  ...
  if (query->currentTime > 0) {
    ...
    if (keyList.size() > 0)
      cout << "Pruning " << keyList.size() << " expired keys ..." << endl;
  ...
  for (size_t i = 0; i < keyList.size(); i++) {
    // cout << "Delete key with Id: " << keyList[i] << " for " << query->userId
    // << endl;
...
const string OpenABEKeystoreManager::searchKey(OpenABEKeyQuery* query,
  ...
  /* error since no key was found -- indicates that a key request needs to be
   * formed */
  return "";
}
